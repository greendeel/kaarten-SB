...
const RoundView: React.FC<RoundViewProps> = ({ 
  ...
  useEffect(() => {
    if (isScoring && tablesContainerRef.current) {
      setTimeout(() => {
        const navHeight = 120; 
        const elementPosition = tablesContainerRef.current?.getBoundingClientRect().top ?? 0;
  ...
  const getTableSum = (table: Table) => {
    return table.participantIds.reduce((sum, pid) => sum + (round.scores[pid] || 0), 0);
  };

  const isTableFinished = (table: Table) => {
    return table.participantIds.every(pid => localScores[pid] !== undefined && localScores[pid] !== "");
  };

  const isTableInError = (table: Table) => {
    return isTableFinished(table) && getTableSum(table) !== 0;
  };
  ...
  }, [participants, localScores, round.scores, round.tables]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      } else if (isRoundValid) {
        if (isEventFinished) setIsScoring(false);
        else onFinishRound();
      }
    }
  };
  ...
  const handleMovePlayer = (pid: string, playerName: string) => {
    ...
    if (newTable) {
      ...
      if (!isNaN(num) && num > 0) {
        onUpdateParticipantTable(pid, num);
      }
    }
  };
  ...
  const renderGameSection = (game: GameType) => {
    ...
    return (
      <div key={game} className="space-y-6">
        ...
        {/* Kolom-indeling: 1 of 2 naast elkaar om maximale breedte te behouden voor de namen */}
                    ...
                    return (
                      <div key={pid} className={`flex items-center justify-between ${isScoring ? 'py-3' : 'py-6'} px-8 bg-white hover:bg-slate-50 transition-all gap-6`}>
                        <div className="flex-1 flex items-center gap-4 min-w-0">
                          ...
                          <p className="font-black text-2xl text-slate-800 whitespace-nowrap overflow-hidden text-ellipsis leading-none flex-1">
                            {player.name}
                          </p>
                        </div>
                        {isScoring ? (
                          <input
                            type="number"
                            inputMode="numeric"
                            readOnly={isEventFinished}
                            value={localValue}
                            placeholder="0"
                            onKeyDown={handleKeyDown}
                            onFocus={(e) => e.target.select()}
                            onChange={(e) => handleInputChange(pid, e.target.value)}
                            className={`w-28 h-20 text-4xl font-black text-center border-4 rounded-2xl outline-none shadow-inner shrink-0 ${isEventFinished ? 'bg-slate-100 border-slate-200 text-slate-500' : 'border-slate-100 focus:border-blue-400 bg-slate-50 text-slate-900'}`}
                          />
                        ) : (
                          <div className="w-16 h-16 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center border-2 border-blue-100 shadow-inner shrink-0">
  ...
  return (
    <div className="p-4 max-w-[1600px] mx-auto space-y-10 pb-48">
      <div className="bg-white p-6 rounded-[2.5rem] border-4 border-blue-100 shadow-xl text-center space-y-3 relative">
        ...
        <div className="bg-blue-600 text-white py-2 px-8 rounded-full inline-block text-xl font-black uppercase tracking-widest shadow-md">
          {isScoring ? (isEventFinished ? 'Scores Bekijken' : 'Scores Opschrijven') : 'Zoek uw tafel'}
        </div>
        
        <div className="pt-4 flex justify-center">
          <button 
            onClick={() => { if(confirm('Tafelindeling aanpassen?')) onResetTables(); }}
            className="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-600 px-6 py-3 rounded-2xl border-2 border-slate-200 font-black text-sm uppercase tracking-wider transition-all shadow-sm active:scale-95 cursor-pointer"
          >
            <RefreshCw size={18} />
        ...
        <div className="max-w-md mx-auto pointer-events-auto">
          {!isScoring ? (
            <button
              ...
              className="w-full bg-blue-700 text-white py-4 rounded-[2rem] text-3xl font-black hover:bg-blue-800 shadow-xl border-b-[8px] border-blue-950 active:translate-y-2 active:border-b-4 transition-all uppercase"
            >
              {isEventFinished ? 'SCORES BEKIJKEN' : 'SCORES INVULLEN'}
            </button>
          ) : (
            <div className="w-full space-y-4">
              {anyTableInError && !isEventFinished && (
              ...
              <button
                onClick={() => isEventFinished ? setIsScoring(false) : onFinishRound()}
                disabled={!isRoundValid && !isEventFinished}
                className={`w-full py-4 rounded-[2rem] text-3xl font-black shadow-xl border-b-[8px] transition-all uppercase ${isRoundValid || isEventFinished ? 'bg-green-600 border-green-900 text-white hover:bg-green-700 active:translate-y-2 active:border-b-4' : 'bg-slate-300 border-slate-400 text-slate-500 opacity-50'}`}
              >
                {isEventFinished ? 'TERUG NAAR TAFELS' : 'VOLGENDE STAP'}
              </button>
            </div>
          )}
        </div>
